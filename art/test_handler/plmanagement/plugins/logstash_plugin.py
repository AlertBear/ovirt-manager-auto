...
from art.test_handler.plmanagement.plugins.jenkins_summary_logger import (
    getLogger,
)
...
class LogStash(Component):
    ...
    def configure(self, params, conf):
        ...
        self.xmllogger = getLogger(
            'logstash',
            conf[LOGSTASH_SEC][SUMMARY_XML_DIR],
        )
        self.xmllogger.field(
            name='What do the colors on the test title mean?',
            cdata=(
                '<p align="center"><font color="#990000">Failed</font>'
                '<font color="#999900">Skipped</font>'
                '<font color="#000099">Passed</font></p>'
            ),
        )
    ...
    def __generate_link(self, machine, name, log_path, st, et):
        ...
        return name, self.__get_query(
            st, et, source_host=machine, source_path=log_path,
        )
    ...
    def __summary_results(self, title, st, et, status='Passed'):
        """
        Creates a new field and a new table with the logstash links
        for the test case/suite.
        """
        # make sure we are not inside a table,
        # if we are not this will do nothing
        et = self.__format_time(et)
        st = self.__format_time(st)
        ...
        for name, path in self.vdc_logs.items():
            self.xmllogger.table_add_column(
                path,
                href=self.__get_query(st, et, source_path=path),
            )
            self.xmllogger.table_add_cell(
                'ENGINE:%s' % self.vdc,
                path,
                '%s:%s' % (self.vdc, path),
                href=self.__get_query(
                    st, et, source_host=self.vdc, source_path=path,
                ),
            )
        for vds in self.vds:
            for name, path in self.vds_logs.items():
                ...
                self.xmllogger.table_add_cell(
                    ...
                    path, '%s:%s' % (vds, path),
                    href=self.__get_query(
                        st, et, source_host=vds, source_path=path,
                    ),
                )
    ...
    def __get_query(self, st, et, **conditions):
        ...
        q = (
            'AND '.join('@%s:%s ' % it for it in conditions.iteritems())
    ...
    def fill_setup_params(cls, params):
        ...
        params['py_modules'] = [
            'art.test_handler.plmanagement.plugins.logstash_plugin',
            'art.test_handler.plmanagement.plugins.jenkins_summary_logger',
        ]

    def config_spec(self, spec, val_funcs):
        section_spec = spec.setdefault(LOGSTASH_SEC, {})
        section_spec[SITE] = "string(default='%s')" % DEFUALT_SITE
        section_spec[ENABLED] = "boolean(default=False)"
        vdc_section = section_spec.setdefault(VDC_LOGS, {})
        vdc_section['engine'] = "string(default='%s')" % DEFAULT_VDC_LOG
        vds_section = section_spec.setdefault(VDS_LOGS, {})
        vds_section['vdsm'] = "string(default='%s')" % DEFAULT_VDS_LOG
        section_spec[SUMMARY_XML_DIR] = (
            "string(default='%s')" % DEFAULT_SUMMARY_XML_DIR
        )

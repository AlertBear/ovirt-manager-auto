...
def check_vm_migration(
    vm_names, orig_host, vm_user, host_password, vm_password, os_type,
    dest_host=None, nic=None, timeout=300, maintenance=False
):
    """
    ...
    :type vm_user: str
    :param host_password: Host password
    ...
    :type nic: str
    :param timeout: Timeout for check VM connectivity
    :type timeout: int
    :param maintenance: Start migration by set original host to maintenance
    ...
    if maintenance and nic:
        LOGGER.error("Can only get maintenance or nic")
        return False

    # causes VM migration by turning down NIC with required network
    if maintenance or nic:
        if maintenance:
            LOGGER.info("Setting %s into maintenance", orig_host)
            if not hosts.deactivateHost(positive=True, host=orig_host):
                LOGGER.error("Failed to set %s into maintenance", orig_host)

        if nic:
            LOGGER.info("Setting %s down on %s", nic, orig_host)
            if not hosts.setHostToNonOperational(
                orig_host=orig_host, host_password=host_password, nic=nic
            ):
                LOGGER.error(
        ...
        for vm in vm_names:
            LOGGER.info("Wait till %s come UP after migration", vm)
            if not vms.waitForVMState(
                vm=vm, sleep=INTERVAL, timeout=TIMEOUT
            ):
            ...
            if out['vmHoster'] == orig_host:
                LOGGER.error(
                    "VM %s is located on original host after migration", vm
                )
                return False
        if nic:
            ...
            LOGGER.info(
                "Put the %s in the UP state and activate the %s", nic,
                orig_host
            )
            ip = hosts.getHostIP(orig_host)
            if not hosts.ifupNic(
                host=ip, root_password=host_password, nic=nic, wait=False
            ):
                LOGGER.error(
                    "Couldn't put NIC %s in up state on %s", nic, orig_host
                )
                return False
    ...
    # call migrateVm function to migrate the VMs one by one
        ...
        try:
            vms.migrate_vms(vms_list=vm_names, dst_host=dest_host)
        except errors.VMException:
            LOGGER.error("Failed to migrate %s", vm_names)
            return False
    ...
    for vm in vm_names:
        ...
        if dest_host:
            if out["vmHoster"] != dest_host:
                LOGGER.error(
                    "VM %s isn't located on destination host after "
                    "migration", vm
                )
                return False
        else:

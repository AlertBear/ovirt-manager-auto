...
logger = logging.getLogger("add_vm_test")
...
class AddVm(VirtTest):
    ...
    def test_custom_boot_sequence(self):
        ...
        self.assertTrue(
            boot_list[0] == config.ENUMS['boot_sequence_network'] and
    ...
    def test_add_vm_with_long_name_crud(self):
        ...
        self.assertTrue(
            helper.create_vm_from_template(
                vm_name=vm_name,
                cluster=cluster_name,
                template=template_name),
            "Failed to add vm with 64 characters"
        )
        self.assertTrue(
            ll_vms.updateVm(

...
class NetLabels14(LabelTestCaseBase):
    ...
    def setup_class(cls):
        ...
        logger.info("Create new DC and Cluster in the setup of the 3.0 "
                    "version")
        if not (addDataCenter(positive=True, name=cls.uncomp_dc,
                              storage_type=config.STORAGE_TYPE,
...
class NetLabels15(LabelTestCaseBase):
    ...
    def setup_class(cls):
        ...
        if not addDataCenter(positive=True, name=cls.dc_name2,
            ...
            raise NetworkException("Couldn't add a new DC %s to the setup" %
                                   cls.dc_name2)

        logger.info("Create Clusters with all supported version for "
                    "labeling for DC %s ", cls.dc_name2)
        ...
        for index, cluster in enumerate(cls.comp_cl_name):
            ...
            if not createAndAttachNetworkSN(data_center=cls.dc_name2,
                                            cluster=cluster,
                                            network_dict=local_dict):
                raise NetworkException("Cannot create network %s for CLuster "
    ...
    def test_move_host_supported_cl(self):
        ...
        """
        logger.info("Move the Host with labeled interface between all "
                    "Clusters of the 3.1 DC and check that Network labels "
                    "feature works for each Cluster version")
        ...
        for index, cluster in enumerate(self.comp_cl_name):
            ...
            if not updateHost(True, host=config.HOSTS[0],
                              cluster=cluster):
                raise NetworkException("Cannot move host to cluster %s" %
                                       cluster)

            if index is not 0:
                logger.info("Check network %s doesn't reside on Host in "
                            "Cluster %s", config.NETWORKS[index], cluster)
                sample = TimeoutingSampler(timeout=60, sleep=1,
                                           func=net_exist_on_nic)
            ...
            if not add_label(label=config.LABEL_LIST[0],
                             networks=[config.NETWORKS[index]]):
                raise NetworkException("Cannot add label to network %s" %
                                       config.NETWORKS[index])

            logger.info("Check network %s resides on Host in Cluster %s",
                        config.NETWORKS[index], cluster)
            sample = TimeoutingSampler(timeout=60, sleep=1,
                                       func=check_network_on_nic,
...
class NetLabels16(LabelTestCaseBase):
    ...
    def setup_class(cls):
        ...
        """
        logger.info("Create a VM network %s on original DC/Cluster",
                    config.NETWORKS[0])
        local_dict = {config.NETWORKS[0]: {"required": "false"}}
        if not createAndAttachNetworkSN(data_center=config.DC_NAME[0],
                                        cluster=config.CLUSTER_NAME[0],
                                        network_dict=local_dict):
            raise NetworkException("Cannot create VM network %s on DC and "
                                   "CLuster" % config.NETWORKS[0])
        ...
        if not createAndAttachNetworkSN(data_center=config.DC_NAME[0],
                                        ...
                                        network_dict=local_dict):
            raise NetworkException("Cannot create network %s on DC and "
                                   "CLuster %s " % (config.NETWORKS[1],
                                                    cls.cl_name2))

        logger.info("Attach the same label %s to non-VM network %s as in "
                    "original Cluster ", config.LABEL_LIST[0],
                    config.NETWORKS[1])
        if not add_label(label=config.LABEL_LIST[0],
                         networks=[config.NETWORKS[1]]):
            raise NetworkException("Couldn't attach label %s to network "
                                   "%s" % (config.LABEL_LIST[0],
                                           config.NETWORKS[1]))
...
class NetLabels17(LabelTestCaseBase):
    ...
    def setup_class(cls):
        ...
        local_dict = {config.NETWORKS[0]: {"required": "false"},
                      ...
                      config.NETWORKS[2]: {"usages": "",
                                           "required": "false"},
                      config.NETWORKS[3]: {"usages": "",
                                           "required": "false"},
                      config.NETWORKS[4]: {"required": "false"},
                      config.NETWORKS[5]: {"usages": "",
                                           "required": "false"},
                      config.NETWORKS[6]: {"required": "false"},
                      config.NETWORKS[7]: {"vlan_id": config.VLAN_ID[0],
                                           "required": "false"}}
        ...
        if not createAndAttachNetworkSN(data_center=config.DC_NAME[0],
                                        cluster=config.CLUSTER_NAME[0],
                                        network_dict=local_dict):
            raise NetworkException("Cannot create 8 networks on DC and "
                                   "Cluster")
        ...
        for i in range(0, 8, 2):
            logger.info("Attach label %s to networks %s and %s",
                        ...
                        config.NETWORKS[i + 1])
            if not add_label(label=config.LABEL_LIST[i],
                             networks=config.NETWORKS[i:i + 2]):
                raise NetworkException("Couldn't attach label %s to networks"
                                       % config.LABEL_LIST[i])
...
class NetLabels18(LabelTestCaseBase):
    ...
    def setup_class(cls):
        ...
        local_dict = {config.NETWORKS[0]: {"required": "false"},
                      ...
                      config.NETWORKS[3]: {"required": "false",
                                           "usages": ""}}
        ...
        if not createAndAttachNetworkSN(data_center=config.DC_NAME[0],
                                        cluster=config.CLUSTER_NAME[0],
                                        network_dict=local_dict):
            raise NetworkException("Cannot create network %s and %s on DC and "
                                   "Cluster" % (config.NETWORKS[0],
                                                config.NETWORKS[1]))
    ...
    def test_label_restrictioin_vm(self):
        ...
        logger.info("Attach the same label %s to networks %s",
                    config.LABEL_LIST[0], config.NETWORKS[2])
        if not add_label(label=config.LABEL_LIST[0],
                         networks=[config.NETWORKS[2]]):
            raise NetworkException("Couldn't attach label %s to "
                                   "network %s" % (config.LABEL_LIST[0],
                                                   config.NETWORKS[2]))
        ...
        if not add_label(label=config.LABEL_LIST[1],
                         networks=[config.NETWORKS[3]]):
            raise NetworkException("Couldn't attach label %s to network %s" %
                                   (config.LABEL_LIST[1], config.NETWORKS[3]))
...
class NetLabels19(LabelTestCaseBase):
    ...
    def setup_class(cls):
        ...
        logger.info("Create networks %s and %s on original and new Clusters "
        ...
        if not (createAndAttachNetworkSN(data_center=config.DC_NAME[0],
                ...
                createAndAttachNetworkSN(data_center=config.DC_NAME[0],
                                         cluster=cls.cl_name2,
                                         network_dict=local_dict2)):
            raise NetworkException("Cannot create networks %s and %s for"

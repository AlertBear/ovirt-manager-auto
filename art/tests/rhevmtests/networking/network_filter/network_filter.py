...
class NetworkFilterCase02(TestCase):
    ...
    def setup_class(cls):
        ...
        if not addNic(
                positive=True, vm=config.VM_NAME[0], name='nic2',
    ...
    def check_network_filter_on_nic(self):
        ...
        """
        logger.info(
            "Check that Network Filter is enabled for nic2 via dumpxml"
        )
        if not checkNetworkFilteringDumpxml(
                positive=True, host=config.HOSTS_IP[0], user=config.HOSTS_USER,
            ...
            raise NetworkException(
                "Network Filter is disabled for nic2 via dumpxml"
            )
...
class NetworkFilterCase04(TestCase):
    ...
    def check_network_filter_via_ebtables(self):
        ...
        if not checkNetworkFilteringEbtables(
            ...
            raise NetworkException(
                "Network filter is not enabled via ebtables for nic1"
            )
        ...
        if not checkNetworkFilteringEbtables(
            ...
            raise NetworkException(
                "Network filter is not enabled via ebtables for nic2"
            )

        logger.info("hot-unplug nic2 from the VM")
        if not updateNic(
                positive=True, vm=config.VM_NAME[0], nic="nic2",
                plugged="false"
        ):
...
class NetworkFilterCase05(TestCase):
    ...
    def check_network_filter_on_nic(self):
        ...
        if not checkNetworkFilteringEbtables(
            ...
            raise NetworkException(
                "Network filter is enabled via ebtables for nic1"
            )

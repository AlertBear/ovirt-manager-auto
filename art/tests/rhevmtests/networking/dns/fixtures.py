...
def get_hosts_params(request):
    ...
    ovirtmgmt = conf.MGMT_BRIDGE
    spm = ll_hosts.get_spm_host(hosts=conf.HOSTS)
    hosts = filter(lambda x: x != spm, conf.HOSTS)
    dns_conf.WORKING_HOST = hosts[0]
    host_resource = global_helper.get_host_resource_by_name(
        host_name=dns_conf.WORKING_HOST
    )
    net_info = host_resource.network.get_info()
    if net_info.get("bridge") == ovirtmgmt:
        host_ip = net_info.get("ip")
        host_prefix = net_info.get("prefix")
        host_gateway = net_info.get("gateway")
        ...
        assert False, "{bridge} not found on host {host_name}".format(
            bridge=ovirtmgmt, host_name=dns_conf.WORKING_HOST
        )

...
from art.rhevm_api.tests_lib.high_level.networks import\
    createAndAttachNetworkSN, prepareSetup, removeMultiNetworks
...
def setup_package():
    ...
    if config.GOLDEN_ENV:
        ...
        if not hl_vm.start_vm_on_specific_host(vm=config.VM_NAME[0],
                                               host=config.HOSTS[0]):
            raise NetworkException("Cannot start VM %s at host %s" %
...
def teardown_package():
    ...
    if config.GOLDEN_ENV:
        ...
        if not removeMultiNetworks(positive=True,
                                   networks=config.VLAN_NETWORKS[0:5],

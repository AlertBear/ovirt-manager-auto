...
class LinkedCase1(TestCase):
    ...
    def setup_class(cls):
        ...
        if not addNic(
                True, config.VM_NAME[0], name='nic6', network=None,
    ...
    def check_combination_plugged_linked_values(self):
        ...
        for nic_name in ('nic2', 'nic4', 'nic6'):
            self.assertTrue(
                getVmNicLinked(config.VM_NAME[0], nic=nic_name),
        ...
        for nic_name in ('nic2', 'nic3', 'nic6'):
            self.assertTrue(
                getVmNicPlugged(config.VM_NAME[0], nic=nic_name),
...
class LinkedCase2(TestCase):
    ...
    def setup_class(cls):
        ...
        if not addNic(
                True, config.VM_NAME[1], name='nic2',
...
class LinkedCase3(TestCase):
    ...
    def setup_class(cls):
        ...
        if not addNic(
                True, config.VM_NAME[1], name='nic3',
    ...
    def check_ombination_plugged_linked_values(self):
        ...
        self.assertFalse(
            getVmNicLinked(config.VM_NAME[1], nic='nic3'),
        ...
        self.assertTrue(
            getVmNicLinked(config.VM_NAME[1], nic='nic3'),
            "NIC3 is not linked, but it should be"
        )
        ...
        for nic_name in ('nic3', 'nic2'):
            self.assertFalse(
                getVmNicNetwork(config.VM_NAME[1], nic=nic_name),
        ...
        for i in range(2):
            if not updateNic(
                    True, config.VM_NAME[1], "nic%s" % (i + 2),
                    network=config.VLAN_NETWORKS[i],
        ...
        for nic_name in ('nic3', 'nic2'):
            self.assertFalse(
                getVmNicPlugged(config.VM_NAME[1], nic=nic_name),
...
class LinkedCase4(TestCase):
    ...
    def setup_class(cls):
        ...
        if not addNic(
                True, config.VM_NAME[1], name='nic6',
    ...
    def check_start_vm(self):
        ...
        self.assertTrue(
            VM_API.waitForQuery(query, timeout=60, sleep=1),
...
class LinkedCase5(TestCase):
    ...
    def setup_class(cls):
        ...
        if not addNic(
                True, vm=config.VM_NAME[0], name='nic2',
    ...
    def check_port_mirroring_network(self):
        ...
        self.assertTrue(
            updateNic(False, config.VM_NAME[0], "nic2", linked='false'),
            "Unlink NIC2 failed"
        )
        ...
        self.assertTrue(
            updateNic(True, config.VM_NAME[0], "nic2", plugged='true'),
            "Plug NIC2 failed"
        )
...
class LinkedCase6(TestCase):
    ...
    def change_net_param_values(self):
        ...
        self.assertTrue(
            getVmNicLinked(config.VM_NAME[0], nic='nic2'),
            "NIC2  isn't linked but should be"
        )
        self.assertFalse(
            getVmNicLinked(config.VM_NAME[0], nic='nic3'),
            "NIC3 is linked but shouldn't be"
        )
        logger.info("Changing the NICs names and Updating opposite link state")
        for i in range(2):
            if not updateNic(
                    True, config.VM_NAME[0], "nic%s" % (i + 2),
                    name="vnic%s" % (i + 2)
            ):
        ...
        self.assertTrue(
            getVmNicLinked(config.VM_NAME[0], nic='vnic3'),
            "VNIC3 isn't linked but should be"
        )
        self.assertFalse(
            getVmNicLinked(config.VM_NAME[0], nic='vnic2'),
        ...
        for nic_name in ('vnic3', 'vnic2'):
            self.assertFalse(
                getVmNicNetwork(config.VM_NAME[0], nic=nic_name),
                "Update %s with empty Net failed" % nic_name
            )
        logger.info(
            "Updating both NICs with its original networks and unplugging "
        ...
        if not (updateNic(
                True, config.VM_NAME[0], "vnic3", name='nic3')
                and
                updateNic(
                True, config.VM_NAME[0], "vnic2", name='nic2')
                ):
...
class LinkedCase7(TestCase):
    ...
    def change_net_param_values(self):
        ...
        if not updateNic(
                True, config.VM_NAME[1], "nic2", name='vnic2',
                network=config.VLAN_NETWORKS[1],
                vnic_profile=config.VLAN_NETWORKS[1], plugged='false'
        ):
            raise NetworkException(
        ...
        self.assertFalse(
            getVmNicPlugged(config.VM_NAME[1], nic='vnic2'),
        ...
        if not updateNic(
                True, config.VM_NAME[1], "vnic2", name='nic2',
                network=config.VLAN_NETWORKS[0],
        ...
        if not waitForVmsStates(
                True, names=config.VM_NAME[1], timeout=120, states='up'
        ):
        ...
        if not updateNic(
                True, config.VM_NAME[1], "nic2", linked='true', plugged='true',
                network=config.VLAN_NETWORKS[1],
                vnic_profile=config.VNIC_PROFILE[0]
        ):

...
from unittest_lib import BaseTestCase as TestCase
...
ALLOC_SCRIPT_LOCAL = 'tests/rhevm/unittests/mom/hostAlloc.py'
CUR = 0
MAX = 1
...
class MOM(TestCase):
    ...
    def ksm_running(self, host, host_user, host_pwd):
        '''
    ...
    def balloon_usage(self, vm_list, host_id=1, multimem=False):
        ...
        inflated = self.wait_for_balloon_change(True, False, vm_list, multimem)

        self.assertTrue(inflated, "Inflation of balloon not working properly")
        logger.info("inflation successful")

    def balloon_usage_negative(self, vm, host_id=1):
        '''
...
class KSM(MOM):
    ...
    def setup_class(cls):
        ...
        logger.info("Runnign KSM tests on host %s memory - %s B",
        ...
        if not storagedomains.waitForStorageDomainStatus(
                True, config.DC_NAME, config.DATA_NAME[0],
                ENUMS['storage_domain_state_active']):
            raise errors.StorageDomainException(
                "Failed to activate storage domain after restart of VDSM")
        logger.info("Cluster memory overcommit percentage set to %d",
                    MEM_OVERCMT)
        ...
        for vm in self.vm_list:
            ...
            if ksm_running is not None:
                if ksm_running:
                    ...
                    logger.info("KSM triggered on %s-th vm", self.threshold[0])
                    break
                else:
    ...
    def test_b_ksm_kickin(self):
        ...
        self.assertTrue(
            self.ksm_running(
                config.HOSTS[0], config.VDS_ADMIN[0], config.HOSTS_PW[0]),
    ...
    def test_c_ksm_migration(self):
        ...
        if not self.ksm_running(
                config.HOSTS[1], config.VDS_ADMIN[1], config.HOSTS_PW[1]):
            logger.warning(
    ...
    def teardown_class(cls):
        ...
        if not clusters.updateCluster(
                True, config.CLUSTER_NAME, mem_ovrcmt_prc=100,
                ksm_enabled=True):
            raise errors.VMException("Failed to update cluster")
...
class Balloon(MOM):
    ...
    def setup_class(cls):
        ...
        '''
        if not clusters.updateCluster(
                True, config.CLUSTER_NAME, ballooning_enabled=True,
                ksm_enabled=False):
            raise errors.VMException("Failed to update cluster")
        # vms for ballooning
    ...
    @tcms('9860', '326211')
    def test_b_balloon_multimemory(self):
        '''
    ...
    @tcms('9860', '326212')
    def test_c_balloon_multiOS(self):
        '''
    ...
    def test_d_balloon_max(self):
        '''
        Negative testcase of balloon with minimum
    ...
    def test_e_balloon_no_agent(self):
        '''
        Negative testcase to test balloon without agent
        '''
        vm = "balloon-1"
        self.vm_list = [vm]
        self.assertTrue(
    ...
    def teardown_class(cls):
        ...
        logger.info("Pids of proceesses allocating memory - %s",
                    " ".join(str(i) for i in cls.pid_list))
        if cls.pid_list:
            host_machine = machine.Machine(
                config.HOSTS[1], config.VDS_ADMIN[1],
                config.HOSTS_PW[1]).util(machine.LINUX)
            host_machine.killProcess(cls.pid_list)

        if not clusters.updateCluster(
                True, config.CLUSTER_NAME, ballooning_enabled=False,
                ksm_enabled=True):

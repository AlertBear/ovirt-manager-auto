...
def setup_package():
    ...
    if not storagedomains.importStorageDomain(
            ...
            host=config.HOSTS[0], address=config.EXPORT_ADDRESS,
            path=config.EXPORT_PATH):
        raise errors.StorageDomainException("Failed to import storage domain")
    ...
    if not storagedomains.waitForStorageDomainStatus(
            True, config.DC_NAME, config.EXPORT_DOMAIN,
    ...
    for vm in [config.RHEL, config.W7, config.W2K]:
        if not vms.importVm(True, vm,
                            export_storagedomain=config.EXPORT_DOMAIN,
                            import_storagedomain=config.DATA_NAME[0],
    ...
    vm_machine = vms.get_vm_machine(
        config.RHEL, config.VM_USER, config.VM_PASSWD)
    ...
    if not setPersistentNetwork(ip['ip'], config.VM_PASSWD):
    ...
    if not templates.createTemplate(True, name=RHEL_TEMPLATE, vm=config.RHEL):
        raise errors.TemplateException("Failed to create template for pool")
    ...
    for name, vm_num, in [
        ...
        if not pools.addVmPool(
                ...
                template=RHEL_TEMPLATE, description=name+" pool"):
            raise errors.VMException("Failed creation of pool for "+name)
        # detach VMs from pool to be editable
        if not pools.detachVms(True, name):
            raise errors.VMException("Failed to detach VMs from "+name+" pool")
    ...
    if not storagedomains.detachStorageDomain(
            True, config.DC_NAME, config.EXPORT_DOMAIN):
        raise errors.StorageDomainException("Failed to detach export domain")
    ...
    for host, pwd in [(config.HOSTS[0], config.HOSTS_PW[0]),
        ...
        host_machine = machine.Machine(host, 'root', pwd).util(machine.LINUX)
        ...
        if not hosts.change_mom_rpc_port(
            ...
            raise errors.HostException("Failed to change RPC port for mom on "
                                       "host %s" % host)
    ...
    if not storagedomains.waitForStorageDomainStatus(
            ...
            ENUMS['storage_domain_state_active']):
        raise errors.StorageDomainException(
            "Failed to activate storage domain after restart of VDSM on hosts")

    host_names = "%s, %s" % (config.HOSTS[0], config.HOSTS[1])
    if not hosts.waitForHostsStates(True, host_names):
        raise errors.HostException("Failed to activate hosts" % host)
...
def teardown_package():
    ...
    storagedomains.cleanDataCenter(True, dc_name, vdc=config.VDC,

...
class NetworkingNegative(unittest.TestCase):
    ...
    def editNetworkInDC(self):
        ...
        # Actions
        loginAsUser(filter_=False)
        assert networks.updateNetwork(False, network=config.NETWORK_NAME1,
    ...
    def attachingDetachingNetworkToFromCluster(self):
        ...
        for net in [config.NETWORK_NAME1, config.NETWORK_NAME2]:
            assert mla.addPermissionsForDataCenter(True, config.USER_NAME,
                    config.MAIN_DC_NAME, role=role.NetworkUser)
            assert mla.addPermissionsForDataCenter(True, config.USER_NAME,
        ...
        for user in [config.USER_NAME, config.USER_NAME2]:
            ...
            assert networks.removeNetworkFromCluster(False, config.NETWORK_NAME1,
                    config.MAIN_CLUSTER_NAME)
            assert networks.addNetworkToCluster(False, config.NETWORK_NAME2,
                    config.MAIN_CLUSTER_NAME)
    ...
    def networkRequiredToNonRequiredAndViceVersa(self):
        ...
        # Actions
        loginAsUser(filter_=False)
        assert networks.updateClusterNetwork(False, config.MAIN_CLUSTER_NAME,
    ...
    def portMirroring(self):
        ...
        assert mla.addPermissionsForDataCenter(True, config.USER_NAME,
                config.MAIN_DC_NAME, role=role.NetworkUser)
        assert mla.addPermissionsForDataCenter(True, config.USER_NAME,
                config.MAIN_DC_NAME, role=role.UserVmManager)
        assert vms.createVm(True, VM_NAME, '', cluster=config.MAIN_CLUSTER_NAME)
        assert vms.addNic(True, VM_NAME, name=NIC_NAME,
                network=config.NETWORK_NAME, interface='virtio')
        ...
        assert vms.addNic(False, VM_NAME, name=NIC_NAME2,
                network=config.NETWORK_NAME, interface='virtio',
                port_mirroring=config.NETWORK_NAME)
    ...
    def attachNetworkToVM(self):
        ...
        assert vms.updateNic(False, VM_NAME, NIC_NAME, network=config.NETWORK_NAME1)
    ...
    def visibleNetworksAndManipulation(self):
        ...
        assert vms.addNic(False, VM_NAME, name=NIC_NAME3,
                network=config.NETWORK_NAME1, interface='virtio')
        assert vms.updateNic(False, VM_NAME, NIC_NAME2, network=config.NETWORK_NAME1)
        ...
        # Everyone NetworkUser permissons, None network is not count
        # (is not shown in /api/networks) + Default DC
        assert len(nets) == 3
        assert vms.updateNic(True, VM_NAME, NIC_NAME2, network=None)
        assert vms.updateNic(False, VM_NAME, NIC_NAME2, network=config.NETWORK_NAME2)

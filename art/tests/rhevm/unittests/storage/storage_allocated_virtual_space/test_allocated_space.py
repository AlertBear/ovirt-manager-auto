...
from art.rhevm_api.tests_lib.low_level.hosts import waitForHostsStates, waitForSPM
...
class BaseCase(TestCase):
    ...
    def setup_class(cls):
        ...
        cls.disk_types = ('thin_provision', 'preallocated')
        cls.disk_sizes = [160 * GB, 7 * GB]
...
class TestCase286775(BaseCase):
    ...
    def perform_action(self):
        ...
        extend_storage_domain(self.master_domain,
                              ...
                              config.HOSTS[0],
                              config.PARAMETERS)
        self.expected_total_size[self.master_domain] += config.EXTEND_SIZE * GB
    ...
    @istest
    @tcms(TCMS_PLAN_ID, tcms_test_case)
    def extend_domain_and_check_details(self):
        """
...
class TestCase321336(BaseCase):
    ...
    def setUp(self):
        ...
        for vm_name in self.vms:
            ...
            thin_provision = vm_name == 'thin_provision'
            disk_format = config.COW_DISK if thin_provision else config.RAW_DISK
            vm_args = {
                'positive': True,
                'vmName': vm_name,
                ...
                'size': config.VM_DISK_SIZE * GB,
                'volumeType': thin_provision,

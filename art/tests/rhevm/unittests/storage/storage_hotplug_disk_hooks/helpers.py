...
def create_vm_with_disks():
    ...
    vms.createVm(
        ...
        volumeType=True, volumeFormat=ENUMS['format_cow'], memory=GB,
        diskInterface=config.INTERFACE_VIRTIO, cpu_socket=config.CPU_SOCKET,
...
class HotplugHookTest(TestCase):
    ...
    def perform_action(self):
        ...
        with ThreadPoolExecutor(max_workers=config.MAX_WORKERS) as executor:
            for disk_name in self.use_disks:
                LOGGER.info("Calling %s on %s" % (self.action[0], disk_name))

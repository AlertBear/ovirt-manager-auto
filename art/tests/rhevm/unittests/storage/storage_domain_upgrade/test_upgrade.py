...
class TestUpgrade(TestCase):
    ...
    def teardown_class(cls):
        ...
        assert llstoragedomains.execOnNonMasterDomains(True, cls.dc_name,
                                            'detach', 'all')
        _, master_name = llstoragedomains.findMasterStorageDomain(True,
                                            cls.dc_name)
        assert llstoragedomains.deactivateStorageDomain(True, cls.dc_name,
        ...
        for i in range(len(config.PARAMETERS.as_list(cls.domain_kw))):
            assert llstoragedomains.removeStorageDomain(True,
                                        cls.sd_name_pattern % i, cls.host)
    ...
    def test_data_center_upgrade(self):
        ...
        assert llvms.unattendedInstallation(True, self.vm_name,
        ...
        assert cobblerRemoveSystem(config.PARAMETERS['cobbler_address'],
                    config.PARAMETERS['cobbler_user'],
        ...
        for sd_obj in sds:
            was_upgraded = llstoragedomains.checkStorageFormatVersion(True,
                                              sd_obj.name,
                                              self.upgraded_storage_format)
...
class TestUpgradeNFS(TestUpgrade):
    ...
    def setup_class(cls):
        ...
        for index, (address, path) in enumerate(zip(
            ...
            assert hlstoragedomains.addNFSDomain(cls.host,
                    cls.sd_name_pattern % index,
                    cls.dc_name, address, path,
...
class TestUpgradeISCSI(TestUpgrade):
    ...
    def setup_class(cls):
        ...
        for index, (lun_address, lun_target, lun) in enumerate(zip(
            ...
            assert hlstoragedomains.addISCSIDataDomain(cls.host,
                    cls.sd_name_pattern % index,
                    cls.dc_name, lun, lun_address, lun_target,
                    storage_format=cls.storage_format)
